...
def antibiotics():
    ...
    if box == "Advocin 180":  # 1
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Treatment of bovine respiratory disease (BRD) caused by Mannheimia haemolytica and Pasteurella multocida. It's effective against gram-negative bacteria."
        )
        st.markdown(" •	Concentration: 180 mg/mL Danofloxacin.")
        st.markdown(
            "•	Dosage: 8 mg/kg as a single dose or 6 mg/kg as 2 injections q24 hrs."
        )
        st.markdown(f"•	meat withdrawal 4 days")
        st.markdown(
            f"❗Do not use in dairy cattle producing milk for human consumption"
        )
        st.divider()
        bw = st.number_input(label="Body weight (kg)", value=100.0, format="%.0f")
        st.markdown(f"**cattle**")
        st.markdown(
            f"give :blue[{bw*8/conc:.1f}mL SC single does] or :blue[{bw*6/conc:.1f}mL SC 2 injections q24 hrs]"
        )
    ...
    if box == "Amphoprim":  # 2
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Bacterial infections of respiratory, urinary, genital & alimentary tract. effective against Gram-positive and Gram-negative bacteria, especially E. coli and Staphylococcus spp"
        )
        st.markdown(
            f"•	NOT effective against: Leptospira spp., Pseudomonas spp., Erysipelothrix spp. "
        )
        st.markdown(
            "•	Concentration: 40 mg/mL trimethoprim and 200 mg/mL sulphadimethylpyrimidine."
        )
    ...
    if box == "Baytril 100":  # 3
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Respiratory disease associated with Mannheimia haemolytica, Pasteurella multocida, Histophilus somni, Mycoplasma spp."
        )
        st.markdown(" •	Concentration: 100 mg/mL (1g/10mL)")
        st.markdown(
            "•	Dosage: 7.5-12.5 mg/kg as a single dose and 2.5-5 mg/kg q24 for 3 to 5 days"
        )
        ...
        st.markdown(
            f"give :blue[{bw*7.5/conc:.1f} - {bw*12.5/conc:.1f} mL SC single does]"
        )
    ...
    if box == "Bivatop 200 LA":  # 4
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Treatment of a broad range of bacterial infections in cattle, including bovine respiratory infection, foot rot, metritis, infectious bovine keratoconjunctivitis"
        )
        ...
        st.markdown(
            f"❗Administer carefully to avoid tissue irritation or muscle damage with intramuscular injections."
        )
        st.divider()
        bw = st.number_input(label="Body weight (kg)", value=100.0, format="%.0f")
        st.markdown(f"**cattle**")
    ...
    if box == "Clamoxyl RTU":  # 5
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Broad-spectrum antibiotic effective against Gram-positive (Streptococcus, Staphylococcus spp.) and may be effective against Gram-negative bacteria (E. coli, Proteus mirabilis)."
        )
    ...
    if box == "Depomycin":  # 7
        st.subheader(
            "Dihydrostreptomycin + Procaine Penicillin (Aminoglycoside + Penicillin)"
        )
        image = Image.open("ruminant_antibiotics/depomycin.jpg")
        st.image(image)
        st.markdown(
            "•	Uses: Respiratory infections, soft tissue infections, and foot rot. Broad-spectrum activity. Penicillin is effective against Gram-positive bacteria, and Dihydrostreptomycin is effective against Gram-negative bacteria (Brucella, Pasteurella, Klebsiella"
        )
        st.markdown(
            "•	Concentration: 200,000 IU/mL procaine penicillin and 200 mg/mL dihydrostreptomycin"
        )
        st.markdown("•	Dosage (cattle): 1 mL/25 kg q24h IM")
        st.markdown("•	Dosage (sheep): 1 mL/20 kg q24h IM")
        st.markdown(
            f"•	meat withdrawal: 15 days, 21 days (cattle offal), 35 days (sheep offal)"
        )
        st.markdown(f"•	milk withdrawal: 4 milkings")
        st.markdown(
            f"❗Procaine toxicity can occur with overdose. Use with caution in animals with kidney disease"
        )
        st.divider()
        bw = st.number_input(label="Body weight (kg)", value=100.0, format="%.0f")
        st.markdown(f"**cattle**")
    ...
    if box == "Doxymycin QA":  # 14
        ...
        st.markdown(
            "•	Dosage (Bacterial infection): 2-4 mL/100 kg IM/slow IV q24 hr for 2-3 days"
        )
        ...
        st.markdown(f"**Cattle**")
        st.markdown(
            f"give :blue[{bw*2/100:.1f} - {bw*4/100:.1f} mL IM/slow IV q24 hr for 2-3 days]"
        )
    ...
    if box == "Draxxin":  # 8
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Respiratory infections, pneumonia. Primarily effective against Gram-negative bacteria (Mannheimia haemolytica, Pasteurella multocida, Histophilus somni) and some Gram-positive bacteria."
        )
        st.markdown("•	Concentration: 100 mg/mL")
    ...
    if box == "Duplocillin":  # 9
        ...
        st.markdown(
            "•	Concentration: 150,000 IU/mL Procaine Penicillin and 150,000 IU/mL Benzathine Penicillin"
        )
        st.markdown("•	Dosage (cattle): 1 mL/25 kg IM q72 hrs")
        st.markdown("•	Dosage (sheep): 1 mL/20 kg IM q72 hrs")
        st.markdown(
            "•	meat withdrawal: 14 days, 70 days (cattle injection site), 56 days (sheep injection site)"
        )
    ...
    if box == "Engemycin 10%":  # 10
        ...
        st.image(image)
        st.markdown(
            "•	Uses: foot rot, tick-borne diseases (gall-sickness/theileriosis), bacterial pneumonia, mastitis, navel/joint ill, wound infection"
        )
        st.markdown("•	Concentration: 100 mg/mL")
        st.markdown(
            "•	Dosage (bacterial): 10 mg/kg (1mL/10kg) IM/slow IV (over 1 min) q24-48h for 3-5 days"
        )
    ...
    if box == "Excede":  # 11
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Respiratory infections, foot rot, bacterial infections. Effective against Gram-negative bacteria (E. coli, Pasteurella multocida) and some Gram-positive organisms (Streptococcus spp.)."
        )
    ...
    if box == "Excenel RTU":  # 12
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Respiratory infections, acute post partum metritis within 14 days, mastitis, foot rot. Effective against Gram-negative bacteria (E. coli, Pasteurella multocida) and some Gram-positive organisms (Streptococcus spp.)."
        )
    ...
    if box == "Hi-Tet 120":  # 13
        ...
        st.markdown(
            f"❗Use cautiously in young animals due to potential effects on bone growth."
        )
        st.divider()
        bw = st.number_input(label="Body weight (kg)", value=100.0, format="%.0f")
        st.markdown(f"**cattle**")
    ...
    if box == "Mildox":  # 14
        ...
        st.markdown("•	Concentration: 100 mg/mL")
        st.markdown(
            "•	Dosage (Bacterial infection): 2-4 mL/100 kg IM/IV q24 hr for 3-5 days"
        )
        st.markdown("•	Dosage (Heartwater): 2 mL/100 kg IM/IV single injection")
        st.markdown("•	Dosage (Anaplasmosis): 4 mL/100 kg IM/IV can repeat once q24 hr")
        st.markdown("•	meat withdrawal: 14 days")
        ...
        st.markdown(f"**Cattle**")
        st.markdown(
            f"give :blue[{bw*2/100:.1f} - {bw*4/100:.1f} mL IM/IV q24 hr for 3-5 days]"
        )
        st.markdown(f"")
        st.markdown(f"**Heartwater**")
    ...
    if box == "Nuflor":  # 15
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Respiratory infections, pneumonia. Broad-spectrum, effective against Gram-negative bacteria (Mannheimia haemolytica, Pasteurella) and Gram-positive bacteria (Staphylococcus, Streptococcus spp.)."
        )
        st.markdown("•	Concentration: 300 mg/mL")
        st.markdown(
            "•	Dosage (cattle): 20 mg/kg (1 ml/15 kg) IM repeat once q48 hr with 16G needle or 40 mg/kg SC single injection"
        )
        ...
        st.markdown(f"**Cattle**")
        st.markdown(
            f"give :blue[{bw*dosage/conc:.1f} mL IM repeat once q48 hr with 16G needle] or :blue[{bw*dosage/conc*2:.1f} mL SC single injection]"
        )
        st.markdown(f"< 10 ml one site")
        st.markdown(f"")
        st.markdown(f"**Sheep**")
    ...
    if box == "Pendistrep":  # 16
        st.subheader(
            "Procaine Penicillin + Dihydrostreptomycin (Penicillin + Aminoglycoside)"
        )
        image = Image.open("ruminant_antibiotics/pendistrep.png")
        st.image(image)
        st.markdown(
            "•	Uses: Treatment of respiratory infections, soft tissue infections, foot rot, urogenital infections, septicaemia. Effective against Gram-positive bacteria (Streptococcus spp., Clostridium spp., Listeria spp.) and Gram-negative bacteria (Brucella spp., Pasteurella spp., Klebsiella spp.)"
        )
        st.markdown(
            "•	Concentration: 200,000 IU/mL Procaine Penicillin + 200 mg/mL Dihydrostreptomycin"
        )
    ...
    if box == "Sulfatrim":  # 17
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Urinary tract infections, skin infections, respiratory infections. Effective against Actinobacillus, Actinomyces, Bordatella, Corynebacterium, Erysipelothrix, E.coli, Klebsiella, Listeria "
        )
    ...
    if box == "Terramycin LA":  # 18
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Long-acting treatment for respiratory infections, pink eye, foot rot, tick-borne diseases (gall sickness, heartwater, Broad-spectrum, effective against both Gram-positive and Gram-negative bacteria (Staphylococcus, E. coli, Pasteurella)."
        )
        st.markdown("•	Concentration: 200 mg/mL")
        st.markdown(
            "•	Dosage: 1 mL/10 kg IM single injection. Can repeat q72 hr (esp. pink eye)"
        )
        st.markdown("•	meat withdrawal: 28 days")
        ...
        st.markdown(f"**Cattle**")
        st.markdown(
            f"give :blue[{bw/10:.1f} mL IM single injection. Can repeat q72 hr]"
        )
        st.markdown(f"< 10 ml one site")
        st.markdown(f"**Sheep/goats**")
        st.markdown(
...
def repro():
    ...
    if box == "Estrumate":  # 1
        ...
        st.image(image)
        st.markdown(
            "•	Uses: Silent heat, chronic endometritis with persistent CL, pyometra, induction of parturition < 10 days, termination of pregnancy < 100 days, mummified foetus, ovarian luteal cyst, controlled breeding "
        )
        ...
        st.markdown(
            f"❗Woman of childbearing age, asthmatics should not handle this product"
        )
