...
def hypokalaemia():
    ...
    st.markdown(f"**maximum number of vials = :blue[{no_vial:.1f} vials] in 1000 mL ringer's**")  
    st.markdown(f"**maximum number of vials = :blue[{no_vial/5:.1f} vials] in 200 mL ringer's**")  
    ...
    vial = st.number_input(label = "number of vials of KCl want to admin in 1000 mL ringer's", value = 0.0, format='%.1f')  
    ...
    vial3 = st.number_input(label = "number of vials of KCl want to admin in 200 mL ringer's", value = 1.0, format='%.1f')  
    drip_rate3 = st.number_input(label = "drip rate (mL/hr)", value = 1.0, format='%.1f')  
    K_rate3 = (vial3*20+4/5)*drip_rate3/200
    max_rate = 0.5*bw3
    st.markdown(f"{vial3} vials in 200 mL ringer's with {drip_rate3} mL/hr drip rate will give the **K admin rate @ {K_rate3:.1f} mmol/hr**")
    st.markdown(f"**max allowable K rate = {max_rate:.1f} mmol/hr**")
...
def parvo():
    ...
    st.markdown(f":grey[hypovolaemic]: give :blue[{bw*100*0.1:.1f} -{bw*100*0.2:.1f} mL] IV up to shock volume :blue[{90*bw}] + leave catheter in (bandage)")
    ...
    st.markdown(f'give tablet: {0.3*bw/10:.2f} - {0.5*bw/10:.2f} tablets 3 times a day or syrup: {0.3*bw/conc:.2f} - {0.5*bw/conc:.2f} mL 3 times a day')
