...
def hypokalaemia():
    ...
    st.markdown(
        f"**maximum number of vials = {no_vial:.1f} vials in 1000 mL ringer's**"
    )
    ...
    vial = st.number_input(
        label="number of vials of KCl want to admin", value=0.0, format="%.1f"
    )
...
def parvo():
    ...
    st.markdown(
        f":grey[hypovolaemic]: give :blue[{bw*100*0.1:.1f} -{bw*100*0.2:.1f} mL] IV  + leave catheter in (bandage)"
    )
    ...
    st.markdown(
        f"give tablet: :blue[{0.3*bw/10:.2f} - {0.5*bw/10:.2f} tablets 3 times a day] or syrup: :blue[{0.3*bw/conc:.2f} - {0.5*bw/conc:.2f} mL 3 times a day]"
    )
    st.markdown(
